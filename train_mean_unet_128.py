...
def run_train():
    ...
    out_dir = '/root/share/project/kaggle/tgs/results/new/unet/fold0-mean'
    initial_checkpoint = \
    ...
    net_teacher = Net().cuda()

    detatch_teacher_parameters(net_teacher)
    zero_teacher_parameters(net_teacher)
    update_teacher_parameters(net_student, net_teacher, 1,0)
    ...
    log.write('  rate   iter   epoch  lamda  |  loss     acc  (   LB) |  loss     acc  (   LB) |  label  unlabel |  label  unlabel  label  unlabel|  \n')
    log.write('--------------------------------------------------------------------------------------------------------------------------------------------------\n')
    ...
    while  iter<num_iters:
        ...
        for (input_l, truth_l, index_l, cache_l), (input_u, truth_u, index_u, cache_u)  in \
            ...
            if iter % iter_valid==0: # and i!=0:
            #if 0:
                net_student.set_mode('valid')
                net_teacher.set_mode('test')
                valid_loss_s = do_valid(net_student, valid_loader)
                valid_loss_t = do_valid(net_teacher, valid_loader)



                print('\r',end='',flush=True)
                log.write('%0.4f  %5.1f  %6.1f  %6.2f |  %0.3f  %0.3f  (%0.3f) |  %0.3f  %0.3f  (%0.3f) |  %0.3f   %0.3f  |  %0.3f  %0.3f    %0.3f  %0.3f  | %s \n' % (\
            ...
            unlabel_loss_weight = 1  #0.5 #0.5 ## <todo> make a good scheduling

            net_student.set_mode('train')
            net_teacher.set_mode('train')
            ...
            logit_t_l = net_teacher(input_l) #data_parallel(net_teacher,input_l) ## <debug only>
            logit_s_u = net_student(input_u) #data_parallel(net_student,input_u) ## <debug only>
            logit_t_u = net_teacher(input_u) #data_parallel(net_teacher,input_u) ## <debug only>

            loss_l = net_student.criterion(logit_s_l, truth_l)
            loss_u = unlabel_loss_weight * criterion_unlabel(logit_s_u, logit_t_u)

            correct_s_l = net_student.metric(logit_s_l, truth_l)
            correct_s_u = net_student.metric(logit_s_u, truth_u)  ## <debug only>
            correct_t_l = net_teacher.metric(logit_t_l, truth_l)  ## <debug only>
            correct_t_u = net_teacher.metric(logit_t_u, truth_u)  ## <debug only>
            ...
            #update_teacher_parameters(net_student, net_teacher, 1-alpha,alpha) #=0.99
            alpha = 0.95 ## <todo> make a good scheduling
            update_teacher_parameters(net_student, net_teacher, 1-alpha, alpha)
            ...
            batch_loss = np.array((
                           ...
                           correct_s_l.item(), correct_s_u.item(), correct_t_l.item(), correct_t_u.item(),
                         ))
            ...
            print('\r%0.4f  %5.1f  %6.1f  %6.2f |  %0.3f  %0.3f  (%0.3f) |  %0.3f  %0.3f  (%0.3f) |  %0.3f   %0.3f  |  %0.3f  %0.3f    %0.3f  %0.3f  | %s' % (\
                         rate, iter/1000, epoch, unlabel_loss_weight,
                         valid_loss_s[0], valid_loss_s[1], valid_loss_s[2],
                         valid_loss_t[0], valid_loss_t[1], valid_loss_t[2],
                         #train_loss[0], train_loss[1], train_loss[2], train_loss[3],train_loss[4], train_loss[5],
